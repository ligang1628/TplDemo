...
namespace TplDemo.Extensions.AOP
{
    ...
    /// 日志拦截
    /// </summary>
    public class TplDemoLogAOP : IInterceptor
    {
        ...
        /// <param name="accessor">HttpContext上下文，获取用户信息</param>
        public TplDemoLogAOP(IHubContext<ChatHub> hubContext, IHttpContextAccessor accessor)
        {
        ...
        public void Intercept(IInvocation invocation)
        {
            ...
            var dataIntercept = "" +
                ...
                $"【携带的参数】：{string.Join(", ", invocation.Arguments.Select(a => (a ?? "").ToString()).ToArray())} \r\n";
            ...
            try
            {
                ...
                if (IsAsyncMethod(invocation.Method))
                {
                    ...
                    {
                        invocation.ReturnValue = InternalAsyncHelper.AwaitTaskWithPostActionAndFinally((Task)invocation.ReturnValue, async () => await SuccessAction(invocation, dataIntercept), ex =>
                           {
                    ...
                    {
                        invocation.ReturnValue = InternalAsyncHelper.CallAwaitTaskWithPostActionAndFinallyAndGetResult(invocation.Method.ReturnType.GenericTypeArguments[0], invocation.ReturnValue, async (o) => await SuccessAction(invocation, dataIntercept, o), ex =>
                              {
                                  LogEx(ex, dataIntercept);
                              });
                    }
